...
class Graph{
    ...
    public:
        ...
        std::vector<Node*> getMaxOuts(void){return this->maxOuts;}
};
...
int main(){
    ...
    OutputFile << "IPS with most Out-Degrees\n";
    ip_graph.checkOutDegree(OutputFile);


    // Get the IP segment with the maximum out degree
    std::vector<Node*> maxOuts = ip_graph.getMaxOuts();
    int maxOutValue = maxOuts[maxOuts.size() - 1]->value;
    std::vector<std::string> entries;

    // Check for entries what contain that ip segment
    std::ifstream InputFile2("bitacora.txt");
    while(InputFile2){
        // Reset variables
        ip = aux = "";
        getline(InputFile2, entry);
        std::stringstream ss(entry);
        ...
        for(int i=0; i < aux.length(); i++){
            ip += aux[i];
            if(aux[i] == '.' || aux[i] == ':'){
                if(std::stoi(aux2) == maxOutValue){contains = true;}
                aux2 = "";  // Reset
                if(aux[i] == ':'){break;}
            }else if(aux[i] != '.' && aux[i] != ':'){
                aux2 += aux[i];
        ...
        if(contains){entries.push_back(entry);}
    }

    OutputFile << "\n\nENTRIES WITH NODE " << maxOutValue << " OUT EDGES\n";
